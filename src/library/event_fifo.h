...
public:
    ...
    inline bool push(Event event) {return _fifo.push(event);}

    inline bool pop(Event& event)
    {
    ...
    virtual void send_event(Event event) override {push(event);}

private:
    memory_relaxed_aquire_release::CircularFifo<Event, MAX_EVENTS_IN_QUEUE> _fifo;
};
